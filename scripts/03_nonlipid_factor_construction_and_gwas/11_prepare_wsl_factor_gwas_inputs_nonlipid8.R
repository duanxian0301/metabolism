...
panel_file <- file.path(root_dir, "final_model_nonlipid_module_final8", "final8_trait_manifest.tsv")
output_dir <- file.path(root_dir, "step21_native_wsl_factor_gwas_inputs_nonlipid8")
...
ref_file <- file.path(output_dir, "nonlipid8_internal_reference.tsv.gz")
...
saveRDS(sumstats_obj, file.path(output_dir, "nonlipid8_factorGWAS_sumstats.rds"))
fwrite(as.data.frame(sumstats_obj), file.path(output_dir, "nonlipid8_factorGWAS_sumstats.tsv.gz"), sep = "\t")
...
chunk_manifest <- data.frame(
  ...
  file_name = paste0("GenSem_sub_", seq_along(sumstats_list), "_final8_nonlipid3F.tsv"),
