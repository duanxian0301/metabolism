...
output_root <- file.path(project_root, "results", "19_knk_lipid8_F2_AD_core4")
...
term2gene <- rbindlist(lapply(names(Genes_by_pathway_kegg), function(pw) {
  data.table(pathway_id = pw, gene = Genes_by_pathway_kegg[[pw]])
}))

queue <- rbindlist(list(
  data.table(dataset = "MSSM_AD", group = "Pericyte", gene = "KANSL1", priority = "tier1", rationale = "scPagwas-significant pericyte plus Bryois AD/F2 shared support"),
  data.table(dataset = "MSSM_AD", group = "Oligodendrocyte_precursor_cell", gene = c("ARL17B", "LRRC37A2"), priority = "tier1", rationale = "scPagwas-suggestive OPC plus Bryois AD/F2 shared high-convergence support"),
  data.table(dataset = "MSSM_AD", group = "Oligodendrocyte_precursor_cell", gene = "KNOP1", priority = "supplement", rationale = "Additional OPC Bryois AD/F2 shared support")
))
if (shard_n > 1L) {
...
read_group_to_seurat <- function(group_dir) {
  mtx <- readMM(file.path(group_dir, "matrix.mtx.gz"))
  barcodes <- fread(file.path(group_dir, "barcodes.tsv.gz"), header = FALSE)$V1
  features <- fread(file.path(group_dir, "features.tsv.gz"), header = FALSE)
  metadata <- fread(file.path(group_dir, "metadata.csv.gz"), data.table = FALSE)
...
run_one <- function(dataset, group, gene, priority, rationale) {
  ...
  req_files <- file.path(group_dir, c("matrix.mtx.gz", "barcodes.tsv.gz", "features.tsv.gz", "metadata.csv.gz"))
  if (!all(file.exists(req_files))) {
    log_msg(logfile, "Skipping: missing prepared group input")
    return(data.table(
      disease_dataset = dataset, cell_type = group, gene = gene,
      input_cell_number = NA_integer_, ko_success = FALSE, perturbed_gene_n = NA_integer_,
      top_pathways = NA_character_, remarks = "Missing prepared group input"
    ))
  }
  ...
  if (!(gene %in% rownames(counts0))) {
    ...
    return(data.table(
      disease_dataset = dataset, cell_type = group, gene = gene,
      input_cell_number = ncol(obj), ko_success = FALSE, perturbed_gene_n = NA_integer_,
      top_pathways = NA_character_, remarks = "Gene not in expression matrix"
    ))
  }
  if (ncol(obj) < 1000) {
    log_msg(logfile, "Skipping: too few cells for stable KNK")
    return(data.table(
      disease_dataset = dataset, cell_type = group, gene = gene,
      input_cell_number = ncol(obj), ko_success = FALSE, perturbed_gene_n = NA_integer_,
      top_pathways = NA_character_, remarks = "Too few cells"
    ))
  }
  ...
  result <- tryCatch(
    scTenifoldKnk(
      ...
      qc = FALSE,
      nc_nNet = 10,
      nc_nCells = min(500, max(250, floor(ncol(data_knk) / 4))),
      nCores = 2
    ),
    error = function(e) e
  )
  if (inherits(result, "error")) {
    ...
    return(data.table(
      disease_dataset = dataset, cell_type = group, gene = gene,
      input_cell_number = ncol(data_knk), ko_success = FALSE, perturbed_gene_n = NA_integer_,
      top_pathways = NA_character_, remarks = paste("KO failed:", conditionMessage(result))
    ))
  }
  ...
  if (save_full_rds) {
    saveRDS(result, paste0(prefix, "_scTenifoldKnk.rds"))
  }
  ...
  fwrite(data.table(
    ...
    priority = priority,
    rationale = rationale,
    input_cells = ncol(data_knk),
    gene_count = nrow(data_knk),
    perturbed_gene_n = sig_n,
    significant_pathway_n = gsea_info$n_sig
  ), paste0(prefix, "_summary.csv"))

  data.table(
    disease_dataset = dataset, cell_type = group, gene = gene,
    input_cell_number = ncol(data_knk), ko_success = TRUE, perturbed_gene_n = sig_n,
    top_pathways = gsea_info$top, remarks = rationale
  )
}
...
fwrite(summary_rows, file.path(sum_dir, paste0("knk_summary_core4", suffix, ".csv")))
make_convergence(summary_rows)
fwrite(queue, file.path(sum_dir, paste0("knk_execution_queue_core4", suffix, ".csv")))
