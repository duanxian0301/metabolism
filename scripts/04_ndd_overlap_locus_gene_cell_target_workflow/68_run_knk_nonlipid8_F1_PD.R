...
output_root <- file.path(project_root, "results", "24_knk_nonlipid8_F1_PD")
...
queue <- rbindlist(list(
  data.table(dataset = "MSSM_PD", group = "Pericyte", gene = c("LRPAP1", "PRMT7", "GRK4", "DUS2"), priority = "tier1", rationale = "nonlipid8_F1-PD core genes tested in scPagwas-significant pericyte"),
  data.table(dataset = "MSSM_PD", group = "Oligodendrocyte_precursor_cell", gene = c("LRPAP1", "PRMT7", "GRK4", "DUS2"), priority = "tier1", rationale = "nonlipid8_F1-PD core genes tested in scPagwas-significant OPC"),
  data.table(dataset = "MSSM_PD", group = "VIP_GABAergic_cortical_interneuron", gene = c("LRPAP1", "PRMT7", "GRK4", "DUS2"), priority = "tier1", rationale = "nonlipid8_F1-PD core genes tested in scPagwas-significant VIP interneuron")
))
if (shard_n > 1L) {
...
run_one <- function(dataset, group, gene, priority, rationale) {
  ...
  if (ncol(obj) < min_cells_required) {
    ...
    return(data.table(
      disease_dataset = dataset, cell_type = group, gene = gene,
      input_cell_number = ncol(obj), ko_success = FALSE, perturbed_gene_n = NA_integer_,
      top_pathways = NA_character_, remarks = paste("Too few cells:", ncol(obj))
    ))
  }
  ...
  result <- tryCatch(
    scTenifoldKnk(
      ...
      qc = FALSE,
      nc_nNet = 10,
      nc_nCells = min(500, max(200, floor(ncol(data_knk) / 4))),
      nCores = 1
    ),
    error = function(e) e
  )
...
fwrite(summary_rows, file.path(sum_dir, paste0("knk_summary_nonlipid8_F1_PD", suffix, ".csv")))
make_convergence(summary_rows)
fwrite(queue, file.path(sum_dir, paste0("knk_execution_queue_nonlipid8_F1_PD", suffix, ".csv")))
