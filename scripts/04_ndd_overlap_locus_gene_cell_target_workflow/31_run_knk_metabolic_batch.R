...
output_root <- file.path(project_root, "results", "19_knk_lipid8_F2_AD")
...
queue <- rbindlist(list(
  data.table(dataset = "MSSM_AD", group = "Pericyte", gene = c("CAB39L", "KNOP1", "ARL17B", "MAP1LC3A"), priority = "core", rationale = "Core pleiotropic genes in scPagwas-significant pericyte"),
  data.table(dataset = "MSSM_AD", group = "Oligodendrocyte_precursor_cell", gene = c("CAB39L", "KNOP1", "ARL17B", "MAP1LC3A"), priority = "core", rationale = "Core pleiotropic genes in scPagwas-suggestive oligodendrocyte precursor cell")
))
if (shard_n > 1L) {
...
fwrite(summary_rows, file.path(sum_dir, paste0("knk_summary_metabolic", suffix, ".csv")))
make_convergence(summary_rows)
fwrite(queue, file.path(sum_dir, paste0("knk_execution_queue_metabolic", suffix, ".csv")))
