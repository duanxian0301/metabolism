...
OUT_ROOT = ROOT / "results" / "15_smr_bryois_lipid8_F2_AD"
...
def out_prefix(trait: str, celltype: str, chrom: int) -> Path:
    return OUT_ROOT / "single_trait" / trait / "bryois2022_celltype" / celltype / f"{trait}_{celltype}_chr{chrom}"


def output_done(trait: str, celltype: str, chrom: int) -> bool:
    out = Path(str(out_prefix(trait, celltype, chrom)) + ".smr")
    log = Path(str(out_prefix(trait, celltype, chrom)) + ".run.log")
...
def build_cmd(trait: str, celltype: str, chrom: int) -> list[str]:
    gwas = SUMSTAT_DIR / f"{trait}.smr.txt"
    ...
    out = out_prefix(trait, celltype, chrom)
...
def run_one(trait: str, celltype: str, chrom: int) -> int:
    out = out_prefix(trait, celltype, chrom)
    log = Path(str(out) + ".run.log")
    cmd = build_cmd(trait, celltype, chrom)
...
def write_status(tasks: list[tuple[str, str, int]]) -> None:
    ...
    rows = [
        {
            ...
            "celltype": cell,
            "chrom": chrom,
            "done": output_done(trait, cell, chrom),
            "output": str(Path(str(out_prefix(trait, cell, chrom)) + ".smr")),
        }
...
def queue(args: argparse.Namespace) -> None:
    ...
    while True:
        pending = [t for t in tasks if not output_done(*t)]
        write_status(tasks)
        if not pending:
        ...
        for trait, cell, chrom in pending:
            if count_active_smr() >= args.max_total_smr:
                break
            subprocess.Popen(
                [sys.executable, __file__, "single", "--trait", trait, "--celltype", cell, "--chrom", str(chrom)],
...
def main() -> None:
    ...
    p_single.add_argument("--trait", required=True, choices=TRAITS)
    ...
    p_queue.add_argument("--traits", nargs="+", default=TRAITS, choices=TRAITS)
    ...
    if args.mode == "single":
        code = run_one(args.trait, args.celltype, args.chrom)
