...
OUT_ROOT = ROOT / "results" / "14_smr_gtex_lipid8_F2_AD"
...
def out_prefix(trait: str, tissue: str) -> Path:
    return OUT_ROOT / "single_trait" / trait / "gtex_v8_brain" / f"{trait}_{tissue}"


def output_done(trait: str, tissue: str) -> bool:
    out = Path(str(out_prefix(trait, tissue)) + ".smr")
    log = Path(str(out_prefix(trait, tissue)) + ".run.log")
...
def build_cmd(trait: str, tissue: str) -> list[str]:
    gwas = SUMSTAT_DIR / f"{trait}.smr.txt"
    ref = GTEX_BRAIN_DIR / f"{tissue}.lite"
    out = out_prefix(trait, tissue)
...
def run_one(trait: str, tissue: str) -> int:
    out = out_prefix(trait, tissue)
    log = Path(str(out) + ".run.log")
    cmd = build_cmd(trait, tissue)
...
def queue(args: argparse.Namespace) -> None:
    pending = [(trait, tissue) for trait in args.traits for tissue in args.tissues if not output_done(trait, tissue)]
    ...
    while pending:
        ...
        for trait, tissue in pending:
            if output_done(trait, tissue):
                continue
            if count_active_smr() >= args.max_total_smr:
                next_pending.append((trait, tissue))
                continue
            child_cmd = [sys.executable, __file__, "single", "--trait", trait, "--tissue", tissue]
                ...
                rows.append(
                    {
                        "trait": trait,
                        "tissue": tissue,
                        "done": output_done(trait, tissue),
                        "output": str(Path(str(out_prefix(trait, tissue)) + ".smr")),
                    }
                )
        import pandas as pd
...
def main() -> None:
    ...
    if args.mode == "single":
        code = run_one(args.trait, args.tissue)
