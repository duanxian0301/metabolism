...
def summarize_bryois_shared_by_gene(path: Path, trait_a: str, trait_b: str) -> pd.DataFrame:
    df = pd.read_csv(path, sep="\t")
    df["p_SMR"] = pd.to_numeric(df["p_SMR"], errors="coerce")
    if "p_HEIDI" in df.columns:
        df["p_HEIDI"] = pd.to_numeric(df["p_HEIDI"], errors="coerce")
    ...
    any_best = (
        df.sort_values(["Gene", "supported", "p_SMR"], ascending=[True, False, True])
        .groupby("Gene", as_index=False)
        .first()[["Gene", "celltype", "topSNP", "p_SMR", "p_HEIDI", "fdr_SMR", "supported"]]
        .rename(
            columns={
    ...
    shared = (
        df[df["supported"]]
        .pivot_table(index=["Gene", "celltype"], columns="trait", values="p_SMR", aggfunc="min")
        .reset_index()
    )
    for col in [trait_a, trait_b]:
        if col not in shared.columns:
            shared[col] = pd.NA
    shared["shared_supported"] = shared[trait_a].notna() & shared[trait_b].notna()
    ...
    if shared.empty:
        shared_best = pd.DataFrame(
            columns=[
                ...
                "shared_n_celltypes",
                "shared_best_context",
                f"shared_best_p_SMR_{trait_a}",
                f"shared_best_p_SMR_{trait_b}",
                "shared_best_joint_score",
                "shared_supported",
            ]
        )
    else:
        shared["joint_score"] = shared[[trait_a, trait_b]].max(axis=1)
        shared_celltypes = (
        ...
        shared_best = (
            shared.sort_values(["Gene", "joint_score", trait_a, trait_b, "celltype"], ascending=[True, True, True, True, True])
            .groupby("Gene", as_index=False)
            .first()[["Gene", "celltype", trait_a, trait_b, "joint_score"]]
            .rename(
                columns={
                    "celltype": "shared_best_context",
                    trait_a: f"shared_best_p_SMR_{trait_a}",
                    trait_b: f"shared_best_p_SMR_{trait_b}",
...
def main() -> None:
    ...
    ap.add_argument("--disease-trait", required=True)
    ap.add_argument("--fuma-job-dir", required=True)
    ap.add_argument("--fuma-gene2func-dir", required=True)
    args = ap.parse_args()

    pair = args.pair
    factor = args.factor_trait
    disease = args.disease_trait

    outdir = RESULTS / f"17_candidate_gene_integration_{pair}"
    outdir.mkdir(parents=True, exist_ok=True)

    fuma_job_dir = Path(args.fuma_job_dir)
    fuma_gene2func_dir = Path(args.fuma_gene2func_dir)
    coloc_dir = RESULTS / f"09_coloc_{pair}"
    pwcoco_dir = RESULTS / f"10_pwcoco_{pair}"
    smr_brainmeta_dir = RESULTS / f"11_smr_{pair}" / "summary"
    smr_gtex_dir = RESULTS / f"14_smr_gtex_{pair}" / "summary"
    smr_bryois_dir = RESULTS / f"15_smr_bryois_{pair}" / "summary"
    ctwas_dir = RESULTS / f"16_ctwas_{pair}" / "summary"

    fuma_genes = pd.read_csv(fuma_job_dir / "genes.txt", sep="\t")
    fuma_gene_table = pd.read_csv(fuma_gene2func_dir / "geneTable.txt", sep="\t")
    magma = pd.read_csv(fuma_job_dir / "magma.genes.out", sep=r"\s+")
    coloc = pd.read_csv(coloc_dir / f"coloc_{pair}_regions.tsv", sep="\t")
    pwcoco_regions = pd.read_csv(pwcoco_dir / f"coloc_pwcoco_{pair}_region_integrated.tsv", sep="\t")

    smr_brainmeta = best_smr_by_gene(smr_brainmeta_dir / f"smr_brainmeta_{pair}_combined.tsv", "trait")
    if "best_context" in smr_brainmeta.columns:
        smr_brainmeta["best_context"] = "BrainMeta_bulk"
    smr_gtex = best_smr_by_gene(smr_gtex_dir / f"smr_gtex_{pair}_combined.tsv", "tissue")
    smr_bryois = summarize_bryois_shared_by_gene(
        smr_bryois_dir / f"smr_bryois_{pair}_combined.tsv", disease, factor
    )

    ctwas_z = []
    ctwas_pip = []
    for trait in [disease, factor]:
    ...
    coords = pd.concat(
        [
            fuma_genes[["symbol", "chr", "start", "end"]].rename(columns={"symbol": "Gene", "chr": "chrnum", "start": "gene_start", "end": "gene_end"}),
            magma[["Gene", "CHR", "START", "STOP"]].rename(columns={"CHR": "chrnum", "START": "gene_start", "STOP": "gene_end"}),
        ],
        ignore_index=True,
    )
    coords = coords.dropna(subset=["Gene", "chrnum", "gene_start", "gene_end"]).groupby("Gene", as_index=False).first()
    ...
    coloc_links = coloc_links[(coloc_links["gene_start"] <= coloc_links["region_end"]) & (coloc_links["gene_end"] >= coloc_links["region_start"])].copy()
    ...
    pw_links = pw_links[(pw_links["gene_start"] <= pw_links["region_end"]) & (pw_links["gene_end"] >= pw_links["region_start"])].copy()
    ...
    master = master.merge(
        smr_bryois.rename(
            columns={
                ...
                f"shared_best_p_SMR_{disease}": f"smr_bryois_best_p_SMR_{disease}",
                f"shared_best_p_SMR_{factor}": f"smr_bryois_best_p_SMR_{factor}",
    ...
    master.to_csv(outdir / f"{pair}_cross_evidence_master.tsv", sep="\t", index=False)
    shortlist.to_csv(outdir / f"{pair}_cross_evidence_shortlist.tsv", sep="\t", index=False)

    counts = master.groupby("priority_tier").size().reset_index(name="n_genes").sort_values("priority_tier")
    counts.to_csv(outdir / f"{pair}_cross_evidence_tier_counts.tsv", sep="\t", index=False)

    with pd.ExcelWriter(outdir / f"{pair}_cross_evidence_tables.xlsx") as writer:
