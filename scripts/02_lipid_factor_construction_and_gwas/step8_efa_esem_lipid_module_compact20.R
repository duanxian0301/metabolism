...
ldsc_all_dir <- file.path(root_dir, "step7_ldsc_lipid_module_compact20")
output_dir <- file.path(root_dir, "step8_efa_esem_lipid_module_compact20")
...
load_all_covariance <- function() {
  S <- as.matrix(
    read.csv(
      file.path(ldsc_all_dir, "lipid_module_compact20_S_matrix.csv"),
      row.names = 1,
      check.names = FALSE
    )
  )
...
run_or_load_odd_ldsc <- function() {
  odd_rds <- file.path(output_dir, "lipid_module_compact20_ODD_EFA.rds")
  if (file.exists(odd_rds)) {
    message("Reusing existing ODD lipid-module LDSC result...")
    return(readRDS(odd_rds))
  }

  message("Running ODD-chromosome LDSC for lipid-module EFA...")
  odd_out <- ldsc(
    ...
    ldsc.log = file.path(output_dir, "lipid_module_compact20_ODD_EFA"),
    select = "ODD"
  )
...
write.csv(raw_S_odd, file.path(output_dir, "lipid_module_compact20_ODD_S_matrix_raw.csv"), row.names = TRUE)
write.csv(smoothed_S_odd, file.path(output_dir, "lipid_module_compact20_ODD_S_matrix_smoothed.csv"), row.names = TRUE)
write.csv(raw_rg_odd, file.path(output_dir, "lipid_module_compact20_ODD_rg_matrix_raw.csv"), row.names = TRUE)
write.csv(smoothed_rg_odd, file.path(output_dir, "lipid_module_compact20_ODD_rg_matrix_smoothed.csv"), row.names = TRUE)

S_all <- load_all_covariance()
write.csv(S_all, file.path(output_dir, "lipid_module_compact20_ALL_S_matrix_smoothed.csv"), row.names = TRUE)
...
fwrite(criteria_dt, file.path(output_dir, "lipid_module_compact20_factor_criteria.tsv"), sep = "\t")
...
esem_results <- list(
  ...
  fit_target_esem(
    S = smoothed_S_odd,
    loadings_file = file.path(output_dir, "EFA_minres_4factor_loadings.tsv"),
    nfactors = 4,
    dataset_label = "ODD"
  ),
  fit_target_esem(
...
fwrite(summary_dt, file.path(output_dir, "lipid_module_compact20_esem_target_summary.tsv"), sep = "\t")

message("Lipid-module compact20 EFA + target-rotation ESEM finished.")
