...
supp_dir <- file.path(root_dir, "supplement_lipid_final8")
dir.create(supp_dir, showWarnings = FALSE, recursive = TRUE)

manifest_file <- file.path(root_dir, "final_model_lipid_module_final8", "final8_trait_manifest.tsv")
esem_summary_file <- file.path(root_dir, "step13_efa_esem_lipid_module_final8", "lipid_module_final8_esem_summary.tsv")
loadings_file <- file.path(root_dir, "step13_efa_esem_lipid_module_final8", "ALL_3factor_loadings.tsv")
std_meta_file <- file.path(root_dir, "step14_native_wsl_usergwas_final8_results", "merged_lipid_final8", "standard_txt", "lipid_final8_standard_txt_metadata.tsv")
ldsc_log_file <- file.path(root_dir, "step15_ldsc_validation_lipid_final8", "lipid_final8_factor_ldsc_ldsc.log")
ldsc_summary_file <- file.path(root_dir, "step15_ldsc_validation_lipid_final8", "lipid_final8_factor_ldsc_summary.tsv")
rg_matrix_file <- file.path(root_dir, "step15_ldsc_validation_lipid_final8", "lipid_final8_factor_rg_matrix.tsv")
...
fwrite(manifest_dt, file.path(supp_dir, "Supplementary_Table_S1_lipid_final8_trait_manifest.tsv"), sep = "\t")
fwrite(esem_summary_dt, file.path(supp_dir, "Supplementary_Table_S2_lipid_final8_esem_fit.tsv"), sep = "\t")
fwrite(loadings_dt, file.path(supp_dir, "Supplementary_Table_S3_lipid_final8_all3factor_loadings.tsv"), sep = "\t")
fwrite(std_meta_dt, file.path(supp_dir, "Supplementary_Table_S4_lipid_final8_factor_gwas_file_manifest.tsv"), sep = "\t")
...
parse_bivariate <- function(lines) {
  ...
  for (i in seq_along(idx)) {
    ...
    trait_line <- block[1]
    pair_txt <- sub("^Results for genetic covariance between:\\s*", "", trait_line)
...
fwrite(supp_s5, file.path(supp_dir, "Supplementary_Table_S5_lipid_final8_univariate_ldsc_detailed.tsv"), sep = "\t")

supp_s6 <- merge(biv_dt, rg_dt, by = c("trait1", "trait2"), all = TRUE)
fwrite(supp_s6, file.path(supp_dir, "Supplementary_Table_S6_lipid_final8_bivariate_ldsc_detailed.tsv"), sep = "\t")
fwrite(rg_matrix_dt, file.path(supp_dir, "Supplementary_Table_S7_lipid_final8_rg_matrix.tsv"), sep = "\t")

overview_lines <- c(
  "Supplementary overview for lipid_final8",
  ...
  paste0("Factor GWAS outputs merged under: ", file.path(native_dir <- file.path(root_dir, "step14_native_wsl_usergwas_final8_results"), "merged_lipid_final8")),
  "Detailed LDSC metrics are in Supplementary_Table_S5 and Supplementary_Table_S6."
)
writeLines(overview_lines, file.path(supp_dir, "lipid_final8_supplement_overview.txt"))
