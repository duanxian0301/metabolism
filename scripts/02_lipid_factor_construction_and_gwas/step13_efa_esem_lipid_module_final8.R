...
panel_dir <- file.path(root_dir, "lipid_module_from_full_manifest", "compact_panel_review", "ultra_pure_3factor_review", "final8_review")
ldsc_all_dir <- file.path(root_dir, "step12_ldsc_lipid_module_final8")
output_dir <- file.path(root_dir, "step13_efa_esem_lipid_module_final8")
ld_ref_dir <- "D:/LDSC/ldsc-master/eur_w_ld_chr"

dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)

panel_dt <- fread(file.path(panel_dir, "lipid_module_ultrapure3_final8_kept.tsv"))
...
run_or_load_odd_ldsc <- function() {
  odd_rds <- file.path(output_dir, "lipid_module_final8_ODD_EFA.rds")
  ...
  odd_out <- ldsc(
    ...
    ldsc.log = file.path(output_dir, "lipid_module_final8_ODD_EFA"),
    select = "ODD"
  )
...
load_all_covariance <- function() {
  S <- as.matrix(read.csv(file.path(ldsc_all_dir, "lipid_module_final8_S_matrix.csv"), row.names = 1, check.names = FALSE))
...
write.csv(raw_S_odd, file.path(output_dir, "lipid_module_final8_ODD_S_matrix_raw.csv"), row.names = TRUE)
write.csv(smoothed_S_odd, file.path(output_dir, "lipid_module_final8_ODD_S_matrix_smoothed.csv"), row.names = TRUE)
write.csv(S_all, file.path(output_dir, "lipid_module_final8_ALL_S_matrix_smoothed.csv"), row.names = TRUE)
...
fwrite(criteria_dt, file.path(output_dir, "lipid_module_final8_factor_criteria.tsv"), sep = "\t")
...
fwrite(rbindlist(results, fill = TRUE), file.path(output_dir, "lipid_module_final8_esem_summary.tsv"), sep = "\t")

message("Final-8 lipid EFA + ESEM finished.")
