...
ascii_work_dir <- "D:/codex/GenomicSEM/metabolic/step15_lipid_final8_ldsc_work"
out_dir <- file.path(root_dir, "step15_ldsc_validation_lipid_final8")
...
trait_names <- c("lipid8_F1", "lipid8_F2", "lipid8_F3")
...
ldsc_out <- ldsc(
  ...
  wld = wld,
  trait.names = trait_names,
  ldsc.log = file.path(out_dir, "lipid_final8_factor_ldsc")
)

saveRDS(ldsc_out, file.path(out_dir, "lipid_final8_factor_ldsc.rds"))
...
fwrite(summary_dt, file.path(out_dir, "lipid_final8_factor_ldsc_summary.tsv"), sep = "\t")
fwrite(rg_dt, file.path(out_dir, "lipid_final8_factor_rg_matrix.tsv"), sep = "\t")
